...
func InsertPost(post model.Post) error {
	...
	sqlstr :="insert into post(username,txt,title,post_time,update_time,comment_num)values(?,?,?,?,?)"
	_,err:=Db.Exec(sqlstr,post.Username,post.Txt,post.Title,post.PostTime,post.UpdateTime,post.CommentNum)
	if err!=nil{
		return err
	}
	return nil
