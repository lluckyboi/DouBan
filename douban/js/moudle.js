...
async function submitWantOrWatched(){
    ...
    save.children[0].addEventListener("click",async() => {
        ...
        }else if(jdsa[0].children[0].checked){
            makerequestData = "watched=1"
        }
        let comment_tag = document.querySelector("#comment_tag")
        makerequestData = makerequestData+"&tags="+comment_tag.value
        let short_comment = document.querySelector("#short_comment")
        let score = 0;
        makerequestData = makerequestData + "&text="+short_comment.value
        ...
        makerequestData += "&score="+score 
        let film_id = window.location.search.substr(1)
        makerequestData +="&"+film_id
        console.log(makerequestData)
        if((jdsa[0].children[0].checked || jdsa[0].children[0].checked) && comment_tag.value != "" && short_comment.value != ""){
            let wanRequest =await SendDataByToken("/shortpost/addshortpost",makerequestData)
            console.log(wanRequest)
        }
        makerequestData = ""
    })
}

export { PostData, Click, FormToString, MakeUrl, LoginByToken,LoginChecked,GetDataByToken,GetData,SendDataByToken,submitWantOrWatched };